...
\usepackage{totcount} % Ensure the package is loaded

% Initialize and register the counter
\newtotcounter{dataset} 
\regtotcounter{dataset} 

% Define how dataset numbers should appear
\renewcommand{\thedataset}{S\arabic{dataset}}

% Define the dataset command
\newcommand{\dataset}[2]{%
  \refstepcounter{dataset}% Increment counter
...
\usepackage{caption}
\captionsetup{
    format=plain, % Standard paragraph format
    font=small, % Adjust font size (small is standard)
    labelfont=bf, % Bold label
    singlelinecheck=false, % Always apply the justification
    margin=5pt % Adds space between text and caption frame
}
\usepackage{subcaption}
\captionsetup[sub]{margin=0pt,font=small,labelfont={rm}}

\usepackage{chngcntr}  % Allows counter resets
%\counterwithin{figure}{section}  % Reset figure numbering per section
\renewcommand{\thefigure}{\thesection.\arabic{figure}} % Section-based figure numbering
...
\usepackage{tablefootnote} % moved here (must be after `rotating`)\usepackage{tikz}
\usepackage{pgfplots}
...
% SI units
\usepackage[detect-all,separate-uncertainty = true,multi-part-units=single]{siunitx}
...
\DeclareSIUnit\mbgl{m\ b.g.l.}
\DeclareSIUnit\masl{m\ a.s.l.}
\DeclareSIUnit\copies{copies}
\DeclareSIUnit\molar{M}
\DeclareSIUnit\TU{TU}
\DeclareSIUnit\ccstp{ccstp}
...
\usepackage{afterpage}   % Ensure figures appear on the next page
